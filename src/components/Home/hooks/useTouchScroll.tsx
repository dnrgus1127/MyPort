    ...
    useEffect(() => {
        ...
        const touchMoverHandler = (e: TouchEvent) => { 
            ...
            // 스크롤이 있는 경우
                ...
                if (overflowScroll > 0 && overflowScroll > window.innerHeight * 0.05) { // 일정 부분( 가로 드래그를 위해서) 초과하면
                    scrollWrapper.current.style.transform = `translateY(calc(${index * -100}% + ${overflowScroll * (dragDistance < 0 ? -1 : 1)}px))` // 초과한 양 만큼 translateY
                }
            }            
        }
        const touchEndHandler = (e: TouchEvent) => {
            if (!scrollWrapper.current) return;
            if (maxScrollTop === 0) { // 스크롤 없는 섹션에서
                const dragDistance = e.changedTouches[0].pageY - startY; // 드래그 거리
                const innerHeight = window.innerHeight; // 화면 길이
                

                if (Math.abs(dragDistance) > innerHeight * 0.10) { // 화면의 일정 부분 이상 드래그 시 다음 섹션으로 전환
            ...
            if (maxScrollTop > 0) { // 스크롤이 있는 섹션
                const dragDistance = e.changedTouches[0].pageY - startY;
                let restScroll = dragDistance < 0 ? maxScrollTop- startScollTop : startScollTop; // 남아있는 스크롤 양
                let overflowScroll = Math.abs(dragDistance) - restScroll; // 초과한 스크롤 양
                if (overflowScroll > 0 && overflowScroll > window.innerHeight * 0.10) { // 일정 부분( 가로 드래그를 위해서) 초과하면
                    ...
                    scrollWrapper.current.style.transition = `transform ${initialTransitionTime * (Math.abs(dragDistance) / window.innerHeight)} ${transition_timing_function}`;
                }
                startY = 0;
                scrollWrapper.current.style.transform = ``;    
            }
