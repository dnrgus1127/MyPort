        ...
        return (
            <ErrorPageLayout>
                <ErrorPageBox>    
                    ...
                    {error.data.msg instanceof Array ? error.data.msg.map((msg :string, idx : number) => <p key={idx} className='msg'>{msg}</p>)  : <p className='msg'>{error.data.msg}</p> }
                    <PrevPageButton onClick={() => {
                        navigate(-1);
                    }}>이전 페이지</PrevPageButton>
                </ErrorPageBox>
            </ErrorPageLayout>
        )
    }
